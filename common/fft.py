...
class FFT:
    ...
    def __init__(self, data):
        self.FREQ = np.linspace(0, data.sample_props.sft, data.sample_props.spt)
        self.KX = np.linspace(-data.sample_props.sfx / 2, data.sample_props.sfx / 2, data.sample_props.spx)
        self.KY = np.linspace(-data.sample_props.sfy / 2, data.sample_props.sfy / 2, data.sample_props.spy)
...
class Mask:
    ...
    def __init__(self, fft, f_range=None, kx_range=None, ky_range=None):
        self.fft = fft
        if f_range is None:
            self.f_range = (0, self.fft.smpl_props.sft / 2)
        else:
            assert f_range[0] < f_range[1], f'{f_range[0]},{f_range[1]}'
            assert f_range[1] < self.fft.smpl_props.sft / 2, f'{f_range[1]},{self.fft.smpl_props.sft / 2}'
            self.f_range = f_range
        if kx_range is None:
            self.kx_range = (-self.fft.smpl_props.sfx / 2, self.fft.smpl_props.sfx / 2)
        else:
            assert kx_range[0] < kx_range[1], f'{kx_range[0]},{kx_range[1]}'
            assert kx_range[1] < self.fft.smpl_props.sfx / 2, f'{kx_range[1]},{self.fft.smpl_props.sfx / 2}'
            self.kx_range = kx_range

        if ky_range is None:
            self.ky_range = (-self.fft.smpl_props.sfy / 2, self.fft.smpl_props.sfy / 2)
        else:
            assert ky_range[0] < ky_range[1], f'{ky_range[0]},{ky_range[1]}'
            assert ky_range[1] < self.fft.smpl_props.sfy / 2, f'{ky_range[1]},{self.fft.smpl_props.sfy / 2}'
            self.ky_range = ky_range

    def __call__(self):
        # data points inside the frequency range set to 1
        mask_f = np.zeros((self.fft.smpl_props.spx, self.fft.smpl_props.spy, self.fft.smpl_props.spt))
        mask_k = copy(mask_f)
        mask_f[
        :,
        :,
